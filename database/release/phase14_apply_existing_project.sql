-- ═══════════════════════════════════════════════════════════════════════════
-- RugTown Phase 14 apply script — apply on existing production project
--
-- Run AFTER Phase 13 (chapter missions + curve v2) is already applied.
...
-- Step 2: Confirm phase_13 chapter_mission_state + rt_recompute_level_v2 exist
-- Step 3: Run this file (same SQL as migrations/20260730_phase14_gameplay_completion.sql)
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  END IF;
  IF to_regprocedure('public.rt_chapter_one_catalog(text)') IS NULL THEN
    RAISE EXCEPTION 'Prerequisite missing: rt_chapter_one_catalog (apply Phase 13 first)';
  END IF;
  IF to_regprocedure('public.rt_recompute_level_v2(bigint)') IS NULL THEN
    RAISE EXCEPTION 'Prerequisite missing: rt_recompute_level_v2 (apply Phase 13 first)';
  END IF;
  ...
  END IF;
  IF to_regclass('public.party_mission_definitions') IS NULL THEN
    RAISE EXCEPTION 'Prerequisite missing: party_mission_definitions (apply Phase 10K first)';
  END IF;
  RAISE NOTICE 'Phase 14 prerequisites OK — applying migration';
END $$;
