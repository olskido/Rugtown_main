-- ═══════════════════════════════════════════════════════════════════════════
-- RugTown Phase 15 apply script — apply on existing production project
--
-- Run AFTER Phase 14 (gameplay completion) is already applied.
-- DO NOT include database/schema.sql or unrelated bootstrap objects.
--
-- Step 1: Run database/diagnostics/inspect_current_supabase_state.sql
-- Step 2: Confirm Phase 14 RPCs + rt_utc_daily_key exist
-- Step 3: Run this file (same SQL as migrations/20260813_phase15_prelaunch_wallet_guild_vault.sql)
-- ═══════════════════════════════════════════════════════════════════════════

DO $$
BEGIN
  IF to_regclass('public.profiles') IS NULL THEN
    RAISE EXCEPTION 'Prerequisite missing: public.profiles (apply bootstrap / schema first)';
  END IF;
  ...
  END IF;
  IF to_regprocedure('public.rt_utc_daily_key(timestamptz)') IS NULL THEN
    RAISE EXCEPTION 'Prerequisite missing: rt_utc_daily_key (apply Phase 10G first)';
  END IF;
  IF to_regprocedure('public.rt_set_mutation_flag()') IS NULL THEN
    RAISE EXCEPTION 'Prerequisite missing: rt_set_mutation_flag (apply Phase 10G first)';
  END IF;
  ...
  END IF;
  IF to_regprocedure('public.rt_normalize_username(text)') IS NULL THEN
    RAISE EXCEPTION 'Prerequisite missing: rt_normalize_username (apply Phase 10J first)';
  END IF;
  IF to_regprocedure('public.claim_mission_reward(uuid)') IS NULL THEN
    RAISE EXCEPTION 'Prerequisite missing: claim_mission_reward (apply Phase 13B first)';
  END IF;
  RAISE NOTICE 'Phase 15 prerequisites OK — applying migration';
END $$;
